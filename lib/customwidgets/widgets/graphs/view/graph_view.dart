...
class _GraphViewState extends State<GraphView>{
  ...
  Widget build(BuildContext context) {
    ...
    if(widget.graphWidget.xAxes == null || widget.graphWidget.yAxes == null || widget.graphWidget.xAxes!.isEmpty || widget.graphWidget.yAxes!.isEmpty) {
      return const Text("Error 404");
    }
    historyDataBloc ??= HistoryDataBloc(dataPoint: widget.graphWidget.graphLines!.first.dataPoint!);
    return BlocBuilder(
      ...
      builder: (BuildContext context, state) {
        ...
        return SfCartesianChart(
          title: ChartTitle(
              text: widget.graphWidget.title ?? "No Title set"
          ),
  ...
  List<LineSeries<_ChartData, DateTime>> getSeries() {
    ...
    for(GraphLine graphLine in widget.graphWidget.graphLines ?? []) {
      ...
      int end = startEndMap["end"].add(const Duration(seconds: 2, milliseconds: 500)).millisecondsSinceEpoch;
      ...
      for(int time in timeKeys) {
        if(dataPoint.historyData.loadedHistory[time]  != null) {
          data.add(_ChartData(DateTime.fromMillisecondsSinceEpoch(time), dataPoint.historyData.loadedHistory[time]?.toDouble() ?? 0.0));
        }
      ...
      series.add(LineSeries<_ChartData, DateTime>(
        ...
        width: 3,
        markerSettings:  MarkerSettings(isVisible: graphLine.showDataDots ?? false),
  ...
  DateTimeAxis _getPrimaryXAxis() {
    ...
    return DateTimeAxis(
        maximum: startEndMap["end"].add(const Duration(seconds: 2, milliseconds: 500)) ,
