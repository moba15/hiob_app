        ...
        child: Column(
          children: [
            Showcase(
              ...
              child: TextField(
                onChanged: (v) => {widget.graphWidget.title = v, if(v.isEmpty) widget.graphWidget.title = null},
      ...
      children: [
        for(GraphAxis a in widget.graphWidget.yAxes ?? [])
          Dismissible(
            ...
            key: ValueKey(a),
            child: _AxisListTile(axis: a, onSave: (axis) {
              setState(() {
                int id = widget.graphWidget.yAxes?.indexWhere((element) => element.id == a.id) ?? 0;
      ...
      children: [
        for(GraphAxis a in widget.graphWidget.xAxes ?? [])
          Dismissible(
            ...
            child: _AxisListTile(axis: a, onSave: (axis) {
              setState(() {
                int id = widget.graphWidget.xAxes?.indexWhere((element) => element.id == a.id) ?? 0;
...
class _AxisListTile extends StatelessWidget {
  ...
  final GraphWidget graphWidget;
  const _AxisListTile({Key? key, required this.axis, required this.onSave, required this.graphWidget}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return ListTile(
      ...
      onTap: ()  {
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) =>  AxisSettingsPage(graphWidget: graphWidget, graphAxis: axis.clone(), onSave: onSave))
        );
      },
    );
  }
...
class _GraphLineListTile extends StatelessWidget {
  ...
  const _GraphLineListTile({Key? key, required this.line, required this.graphWidget, required this.onSave}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return ListTile(
      ...
      onTap: ()  {
        Navigator.push(
            context,
            MaterialPageRoute(builder: (context) =>  LineSettingsPage(graphWidget: graphWidget, graphLine: line.clone(), onSave: onSave))
        );
      },
    );
  }
    ...
    return ExpansionTile(
      ...
      children: [
        for(GraphLine a in widget.graphWidget.graphLines ?? [])
          _GraphLineListTile(line: a.clone(), graphWidget: widget.graphWidget, onSave: (v) {
            int index = widget.graphWidget.graphLines!.indexWhere((element) => element.name == (a.name));
            setState(() {
        ...
        ElevatedButton(
          ...
          onPressed: ()  {
            Navigator.push(
                context,
                MaterialPageRoute(builder: (context) =>  LineSettingsPage(graphWidget: widget.graphWidget, graphLine: widget.graphLine ?? GraphLine(), onSave: (c)  {
                  setState(() {
