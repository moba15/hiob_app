...
class GraphWidget extends CustomWidget {
  ...
  bool? trackBall;
  GraphWidget({required String? name, this.title, this.yAxes, this.xAxes, this.graphLines, this.updateType, this.trackBall}) : super(name: name, type: CustomWidgetType.graph, settings: {}) {
    if(graphLines != null && graphLines!.isNotEmpty && yAxes != null && yAxes!.isNotEmpty && xAxes != null && xAxes!.isNotEmpty) {
  ...
  @override
  CustomWidget clone() {
    return GraphWidget(name: name, title: title, yAxes: yAxes, xAxes: xAxes, graphLines: graphLines, updateType: updateType, trackBall: trackBall);
  }

  @override
  CustomWidgetSettingWidget get settingWidget => GraphWidgetSettings(graphWidget: this);
  ...
  factory GraphWidget.fromJSON(Map<String, dynamic> json) {
    List<GraphAxis> xAxes = List.of(json["xAxes"]).map((v) => GraphAxis.fromJson(Map.of(v)) ).toList();
    List<GraphAxis> yAxes = List.of(json["yAxes"]).map((v) => GraphAxis.fromJson(Map.of(v)) ).toList();
    return GraphWidget(
      name: json["name"],
      title: json["title"],
      yAxes: yAxes,
      xAxes: xAxes,
      graphLines: List.of(json["graphLines" ] ?? []).map((e) => GraphLine.fromJson(Map.of(e), xAxes, yAxes)).toList(),
