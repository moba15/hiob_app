...
class CustomWebViewWidget extends CustomWidget {
  ...
  bool javaScript;
  DataPoint? dataPoint;
  CustomWebViewWidget({required String? name, required this.url, required this.dataPoint,this.height = 300, this.javaScript = false}) : super(name: name, type: CustomWidgetType.webView, settings: {});
  ...
  factory CustomWebViewWidget.fromJSON(Map<String, dynamic> json) {
    ...
    if(json["dataPoint"] != null) {
      dataPoint = Manager.instance?.deviceManager.getIoBrokerDataPointByObjectID(json["dataPoint"] ?? "");
    } else {
      dataPoint = null;
    }
    return CustomWebViewWidget(name: json["name"],
        url: json["url"] ?? "",
        height: json["height"] ?? 300,
        javaScript: json["javaScript"] ?? false, dataPoint: dataPoint);
  }
    ...
    @override
    CustomWidget clone() {
      return CustomWebViewWidget(name: name, url: url, height: height, javaScript: javaScript, dataPoint: dataPoint);
    }
}
