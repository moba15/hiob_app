...
class BackgroundRunner {
  ...
  static void onStart(ServiceInstance s) {
    ...
    s.on("start").listen((event) {
      ...
      webSocketChannel = IOWebSocketChannel.connect("ws://192.168.178.96:8090"); //TODO Generic
      webSocketChannel!.stream.listen((e) => onData(e, event!["loginPackage"]), onError:  (e) {throw const SocketException("");}, onDone: onDone);
  ...
  static void onData(event,   Map<String, dynamic> requestLoginPackage) {
    ...
    if(packageType == DataPackageType.firstPingFromIob2 || packageType == DataPackageType.firstPingFromIob) {
      //TODO: Login
      webSocketChannel?.sink.add(jsonEncode(
          {"type": DataPackageType.requestLogin.name, "content": requestLoginPackage}));
      ...
      //TODO: Send Data to clarify intend (only notification)
      webSocketChannel?.sink.add(jsonEncode(
          {"type": DataPackageType.notification.name, "content": {"onlySendNotification": true}}));
    }
