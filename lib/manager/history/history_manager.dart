...
class HistoryManager {
  ...
  void subscribeToHistorySmart(DataPoint dataPoint, int start, int end, int interval) {

    if(subscriptions.containsKey(dataPoint)) {
      if(subscriptions[dataPoint]!.any((element) => element.type == SubType.smart)) {
  ...
  void sendSubscription(DataPoint dataPoint, int start, int end, int interval) {
    ...
    if(!_isIntervalIsLoaded(historyData, start, end)) {
      ...
      if(historyData.historyInterval.start > start || historyData.historyInterval.start < 0) {
  ...
  void onHistoryUpdate({required List data}) {
    ...
    for(Map rawDate in data) {
      ...
      DataPoint dataPoint = subscriptions.keys.firstWhere((element) => element.id == id);
